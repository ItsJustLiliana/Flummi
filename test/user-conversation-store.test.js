...
function cleanupUser(userId) {
    fs.rmSync(getUserFilePath(userId), { force: true });
...
test('user conversation store writes memory under data/global/users', () => {
    ...
    try {
        ...
        assert.equal(getUserFilePath(userId), path.join(usersDir, `${userId}.json`));
        assert.equal(fs.existsSync(path.join(dataDir, 'global', 'users', `${userId}.json`)), true);
